		...
		foreach($value as $current_key => $current_value){
			...
			foreach($this->choices as $list_key => $list_value){
				...
				if($field_type == 'text'){
					...
					$output .= '<input type="text" '.$this->get_link().' value="'.$field_value.'" name="'.$field_name.'" id="'.$field_name.'"'.$field_placeholder.'/>';
					
				}elseif($field_type == 'select'){
					$field_class = (isset($field_args['class']) ? $field_args['class'] : '');
					$output .= '<select '.$this->get_link().' class="cpometabox_field_select '.$field_class.'" name="'.$field_name.'" id="'.$field_name.'">';
			...
			$output .= '<a href="#" tabindex="-1" class="cpotheme-collection-remove">'.__('Remove', 'cpocore').'</a>';
			$output .= '</div>';
		}
		$output .= '<div>';
		$output .= '<a href="#" class="button cpotheme-collection-add">'.__('Add Row', 'cpocore').'</a>';
		$output .= '</div>';		
		$output .= '</div>';
		echo $output;
	}
