...
if(!function_exists('cpotheme_form_iconlist')){
	function cpotheme_form_iconlist($name, $value, $args = null) {    
		...
		$output .= '<div class="cpotheme-iconlist-heading">Font Awesome</div>';
		$list = cpotheme_metadata_icons();
		foreach ($list as $list_key => $list_value) {
			$checked = null;
			$selected = '';
			if($list_key === $value && $value != '') {
				$checked = ' checked="checked"';
				$selected = ' selected';
			}
			$output .= '<label class="cpotheme-iconlist-item'.$selected.'" style="font-family:\'fontawesome\';" for="'.$name.'_'.htmlentities(stripslashes($list_key)).'">';
			if($list_key == '0') $output .= ' '; else $output .= $list_key;
			$output .= '<input type="radio" name="'.$name.'" id="'.$name.'_'.htmlentities(stripslashes($list_key)).'" value="'.htmlentities(stripslashes($list_key)).'" '.$checked.'/>';        
			$output .= '</label>';        
		}
		$output .= '</div>';
		return $output;
