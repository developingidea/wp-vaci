...
if(!function_exists('cpotheme_gallery_lightbox')){
	...
	function cpotheme_gallery_lightbox($link, $id, $size, $permalink){
		global $post;
		wp_enqueue_style('cpotheme-prettyphoto');
		wp_enqueue_script('cpotheme_prettyphoto');
		if(!$permalink)
...
if(!function_exists('cpotheme_thumbnail_settings')){
	function cpotheme_thumbnail_settings(){
		...
		add_settings_field('cpotheme_portfolio_thumbnails', __('Portfolio Size', 'cpocore'), 'cpotheme_thumbnail_fields', 'media' , 'default', $data);
	}
}
...
if(!function_exists('cpotheme_thumbnail_fields')){
	function cpotheme_thumbnail_fields($args){
		?>
		<legend class="screen-reader-text"><span><?php _e('Portfolio size', 'cpocore'); ?></span></legend>
		<label for="cpotheme_portfolio_width"><?php _e('Max Width', 'cpocore'); ?></label>
		...
		<label for="cpotheme_portfolio_height"><?php _e('Max Height', 'cpocore'); ?></label>
