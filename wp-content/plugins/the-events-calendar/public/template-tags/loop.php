...
if ( class_exists( 'TribeEvents' ) ) {
	...
	function tribe_get_events_title( $depth = true ) {
		...
		$title = __( 'Közelgő események', 'tribe-events-calendar' );
		...
		if ( tribe_is_month() ) {
			$title = sprintf(
				__( 'Esemény ekkor %s', 'tribe-events-calendar' ),
		...
		// day view title
		if ( tribe_is_day() ) {
			$title = __( 'Esemény ekkor', 'tribe-events-calendar' ) . ' ' .
	...
	function tribe_events_list_the_date_headers() {
		...
		if ( $show_headers ) {
			...
			$month_year_format = tribe_get_option( 'monthAndYearFormat', 'Y F' );
